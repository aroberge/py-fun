...
class Viewer(object):
    ...
    def create_fractal(self, width, height):
        '''draw a fractal on a fake canvas'''
        x_values = [self.canvas_to_complex_plane_x(x)
                            for x in range(0, width)]

        cols = [0 for y in range(height, -1, -1)]
        for y in range(height, -1, -1):
            imag = self.canvas_to_complex_plane_y(y)
            rows = ['#FFFFFF' for x in range(0, width)]
            for x in range(0, width):
                real = x_values[x]
                c = complex(real, imag)
                if mandel(c):
    ...
    def draw_fractal(self):
        ...
        cols = self.create_fractal(width = self.canvas_width,
